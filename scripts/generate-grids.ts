// Builds a frozen bank of 10×10 two-per-line region puzzles.
//   node --experimental-strip-types scripts/generate-grids.ts            (v1: campaign)
//   node --experimental-strip-types scripts/generate-grids.ts --version=v2  (v2: formation)
// Ported from the Bullpen double-cow generator: plant a valid layout, join
// units pairwise into regions, grow the borders, then keep only connected,
// unique, logically solvable grids that differ from every earlier bank
// (including rotations and mirrors). Never regenerate a published bank in place.
...
const version = process.argv.find((a) => a.startsWith('--version='))?.slice('--version='.length) ?? 'v1';
const CONFIG = (
  {
    v1: { seed: 0x3c6ef372, file: 'grid-data.ts', name: 'GRIDS' },
    v2: { seed: 0x9e3779b9, file: 'grid-v2-data.ts', name: 'FORMATION_GRIDS' },
  } as Record<string, { seed: number; file: string; name: string }>
)[version];
if (!CONFIG) throw new Error(`Unknown grid bank version ${version}`);
const destination = new URL(`../src/lib/source/${CONFIG.file}`, import.meta.url);
if (existsSync(destination) && !process.argv.includes('--replace-unreleased'))
  throw new Error(`grid-${version} bank already exists; use a new content version.`);
...
if (version !== 'v1')
  for (const g of (await import('../src/lib/source/grid-data.ts')).GRIDS) seen.add(gridFingerprint(g.regions));
const pool: { regions: number[][]; solution: number[][]; difficulty: { tier: number; score: number; steps: number } }[] = [];
...
// Spread picks evenly across the difficulty range so levels ramp smoothly.
const grids = Array.from({ length: COUNT }, (_, i) => ({
  id: `grid-${version}-${String(i + 1).padStart(3, '0')}`,
  ...pool[Math.floor((i * (pool.length - 1)) / (COUNT - 1))],
}));
writeFileSync(
  destination,
  `// Frozen 10x10 two-per-line grids generated by scripts/generate-grids.ts${version === 'v1' ? '' : ` --version=${version}`}.\n` +
    `// Each is unique, connected and logically solvable. Never regenerate in place.\n` +
    `export const ${CONFIG.name}: { id: string; regions: number[][]; solution: number[][]; difficulty: { tier: number; score: number; steps: number } }[] = [\n` +
